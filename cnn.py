...
class CNN:
        ...
        for hidden_activation in hidden_activations:
            for output_activation in output_activations:
                for optimizer in optimizers:
                    for loss in losses:
                        name = hidden_activation + '_' + output_activation + '_' + optimizer + '_' + loss
    ...
    def _create_model(self, *func, x: np.array) -> models.Sequential:
        ...
        # first layer
        model.add(Conv2D(32, (3, 3), padding='same', activation=hidden_activation, input_shape=x.shape[1:]))
        model.add(MaxPooling2D(pool_size=(2, 2)))

        # second layer
        model.add(Conv2D(64, (3, 3), padding='same', activation=hidden_activation))
        model.add(MaxPooling2D(pool_size=(2, 2)))

        # third layer
        model.add(Conv2D(128, (3, 3), padding='same', activation=hidden_activation))
